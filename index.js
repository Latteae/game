...
app.post('/api/signin', (req, res) => {
    ...
    connection.query("SELECT id FROM users WHERE username = ? AND password = ?", [username, password], (err, result) => {
        if (result && result.length > 0) res.json({ userId: result[0].id });
...
app.get('/api/user-info/:userId', (req, res) => {
    connection.query("SELECT username, country FROM users WHERE id = ?", [req.params.userId], (err, result) => res.json(result[0] || {}));
});
